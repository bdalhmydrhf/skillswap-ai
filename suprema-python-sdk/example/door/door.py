...
class DoorSvc:
  ...
  def lock(self, deviceID, doorIDs, doorFlag, normalizeTimer = 0):
    try:
      self.stub.Lock(door_pb2.LockRequest(deviceID=deviceID, doorIDs=doorIDs, doorFlag=doorFlag, normalizeTimer=normalizeTimer))
    except grpc.RpcError as e:
      print(f'Cannot lock the doors: {e}')
      raise

  def unlock(self, deviceID, doorIDs, doorFlag, normalizeTimer = 0):
    try:
      self.stub.Unlock(door_pb2.UnlockRequest(deviceID=deviceID, doorIDs=doorIDs, doorFlag=doorFlag, normalizeTimer=normalizeTimer))
    except grpc.RpcError as e:
